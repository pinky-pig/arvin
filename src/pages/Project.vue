    ...
    <ClientOnly>
      <div class="grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] mx-auto max-w-[970px] justify-center gap-[10px] rounded-xl px-[5px] text-start md:grid-cols-[repeat(auto-fill,minmax(240px,300px))] md:px-[25px]">
        ...
        >
          ...
          >
            <a :href="project.demoUrl" class="inline-flex gap-[0.5] pr-[0.5] text-[0.95em] leading-none hover:underline" target="_blank" data-state="closed" @click="(event: MouseEvent) => event.stopPropagation()">
              {{ project.name }}
